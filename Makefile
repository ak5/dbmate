...
# strip binaries
FLAGS := -tags sqlite_omit_load_extension,sqlite_json,sqlite_fts5 -ldflags '-s'
...
ifeq ($(GOOS),linux)
	...
	FLAGS := -tags netgo,osusergo,sqlite_omit_load_extension,sqlite_json,sqlite_fts5  -ldflags '-s -extldflags "-static"'
endif
ifeq ($(GOOS),darwin)
